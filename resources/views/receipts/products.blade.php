<div class="card mt-4 overflow-hidden">
    <div class="card-body">
        <table class="table table-bordered table-product" data-table="receipt">
            <thead>
                <tr>
                    <th style="width: 5%;">No.</th>
                    <th style="width: 15%;">{{ trans('product.code') }}</th>
                    <th style="width: 20%;">{{ trans('product.name') }}</th>
                    <th style="width: 10%;">{{ trans('receipt.quantity') }}</th>
                    <th>{{ trans('product.unit') }}</th>
                    <th style="width: 13%;">{{ trans('product.price') }}</th>
                    <th style="width: 20%;">{{ trans('product.total_price') }}</th>
                    <th style="width: 8%;"></th>
                </tr>
            </thead>
            <tbody>
                @if (old('products'))
                    ...
                    @endforeach
                @endif
                @for ($i = count(old('products', [])); $i < 20; $i++)
        ...
        <div class="row align-items-center justify-content-end mb-3">
            ...
            <div class="col-3 readonly-custom">
                <x-form::input name="total" :value="number_format(old('price_total_product', ZERO_MONEY))" class="text-end" required readonly />
            </div>
        </div>
        <div class="row align-items-center justify-content-end mb-3">
            <div class="col-auto text-end pe-0">
            ...
            <div class="col-3">
                <x-form::input name="tax" min="0" :value="number_format(old('tax', CONSUMPTION_TAX))" class="text-end input-mark-decimal" required />
            </div>
            ...
            <div class="col-3 readonly-custom">
                <x-form::input name="consumption_tax" :value="number_format(old('consumption_tax', ZERO_MONEY))" class="text-end input-number-validate" required
                    readonly />
            </div>
        </div>
        <div class="row align-items-center justify-content-end ">
            ...
            <div class="col-3 readonly-custom">
                <x-form::input name="total_amount" :value="number_format(old('total_receipt_amount', ZERO_MONEY))" class="text-end" required readonly />
            </div>
        </div>
