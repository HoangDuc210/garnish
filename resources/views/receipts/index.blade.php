@extends('layout')
@section('title') {{ trans('nav.receipt.receipt_management') }} @stop
@section('content')
    <x-top-page :name="trans('nav.receipt.receipt_management')">
    </x-top-page>
    <x-alert />
    @include('receipts.filter')
    <div class="card">
        <div class="card-body">
            {{ $receipts->onEachSide(ADD_PAGE_SIZE)->links() }}
            <x-table.table class="align-middle table-bordered my-3" data-table-checkbox>
                <x-slot:thead>
                ...
                <tbody>
                    @foreach ($receipts as $receipt)
                        <tr data-id="{{ $receipt->id }}">
                            ...
                            <td class="text-end">{{ number_format($receipt->round_total_receipt_amount) }}</td>
                            <td class="text-center">
                                {{ $receipt->print }}
                            </td>
                            <td class="text-center">
                                <div class="status d-flex">
                                    <a href="{{ route(RECEIPT_DETAIL_ROUTE, $receipt->id) }}"
                                        class="btn btn-info btn-circle me-1" data-toggle="tooltip" data-placement="top" title="{{ trans('app.detail') }}">
                                        <i class="fa-regular fa-eye"></i>
                                    </a>
                                    <a href="{{ route(RECEIPT_EDIT_ROUTE, $receipt->id) }}"
                                        class="btn btn-primary btn-circle me-1" data-toggle="tooltip" data-placement="top" title="{{ trans('app.edit') }}">
                                        <i class="fa-regular fa-pen-to-square"></i>
                                    </a>
                                    <button data-href="{{ route(RECEIPT_DELETE_ROUTE, $receipt->id) }}"
