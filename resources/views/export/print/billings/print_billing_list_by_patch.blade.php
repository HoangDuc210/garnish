...
@section('content')
    ...
    <table class="table table-deposit table-bordered mb-4">
        ...
        <tbody>
            <tr>
                <td class="text-end">{{ number_format($lastBilledAmount) }}</td>
                <td class="text-end">{{ number_format($billing?->deposit_amount) }}</td>
                <td class="text-end">{{ number_format($lastBilledAmount - $billing?->deposit_amount) }}
                </td>
                <td class="text-end">{{ number_format($billing?->final_receipt_amount) }}
                </td>
                <td class="text-end">{{ number_format($billing?->tax_amount) }}</td>
                <td class="text-end">
                    {{ number_format($lastBilledAmount - $billing?->deposit_amount + ($billing?->final_receipt_amount + $billing?->tax_amount)) }}
                </td>
